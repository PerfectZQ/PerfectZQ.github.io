...
public class AutomaticOffsetConsumer {
    ...
    public static void main(String[] args) {
        ...
        // 通过`auto.commit.interval.ms`来控制提交`offset`的频率
        props.put("auto.commit.interval.ms", "1000");
        /**
         *  当kafka中没有初始`offset`或如果当前`offset`在服务器上不再存在时，共支持三种消费策略。
         *  注意不论那种策略，如果有当前`group.id`在`topic`的`partition`下有提交的`offset`
         *  则从提交的`offset`开始消费
         *  
         *  earliest: 当无提交的`offset`时，从分区的最开始消费
         *  latest(default): 当无提交的`offset`时，从分区下最新生产的的数据开始消费 
         *  none: 只要有一个分区不存在已提交的`offset`，则抛出异常
         */
