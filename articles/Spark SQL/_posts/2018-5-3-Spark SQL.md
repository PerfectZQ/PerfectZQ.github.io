...
* [Built-in Functions](https://spark.apache.org/docs/latest/sql-ref-functions-builtin.html)
  ...
  * Aggregate-like Functions
    * [Aggregate Functions](https://spark.apache.org/docs/latest/sql-ref-functions-builtin.html#aggregate-functions): 作用于一组记录(通常和`GROUP BY sex`一起使用)，对这一组的数据的列进行聚合计算得出一个值(例如`MAX(age)`)，做聚合后结果表的总记录数通常会减少。
