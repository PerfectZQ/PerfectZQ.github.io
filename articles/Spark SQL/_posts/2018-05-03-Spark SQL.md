...
## SparkSQL 逻辑执行计划优化规则 
* [SparkSQL 源码系列 | 最全的 Logical Plan 优化规则整理](https://cloud.tencent.com/developer/article/2019623?areaId=106001)
...
#### 解决数据倾斜
##### 避免数据 Shuffle
利用 Broadcast Join 避免数据进行 Shuffle，这样数据就不会倾斜(取决于 Map 阶段的数据分布)，但 Broadcast Join 只适合小表，先把小表数据加载到 Driver 内存，然后广播到各个 Executor 节点上，中间不存在数据 Shuffle 过程
