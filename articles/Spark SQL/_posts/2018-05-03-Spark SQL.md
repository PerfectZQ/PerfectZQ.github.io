...
当前 SparkSQL 支持 5 种 JOIN 策略，`Broadcast Hash Join`、`Shuffle Hash Join`、`Shuffle Sort Merge Join`、`Cartesian Product Join`、`Broadcast Nested Loop Join`，其中前两者归根到底都属于`Hash Join`，只不过在`Hash Join`之前需要先 Shuffle 还是先 Broadcast。
