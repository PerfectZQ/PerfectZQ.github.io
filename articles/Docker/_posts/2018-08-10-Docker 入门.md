...
# . 表示上下文环境是当前目录，默认在指定的 contextDir 查找 dockerfile
...
# 注意 source 路径目录是相对于 contextDir 的，并且 docker 只能访问 contextDir 中的文件
# 访问 contextDir 之外的文件会出现 COPY failed: Forbidden path outside the build context: ../test ()
...
-rw-r--r-- 1 root root     104 Mar 21 07:28 log4j2_logs.dockerfile
```
