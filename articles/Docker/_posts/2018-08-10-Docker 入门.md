...
默认情况下，`RUN`指令会在`Shell`里使用命令包装器`/bin/sh -c`来执行，如果是在一个不支持`Shell`的平台上运行或者不希望在`Shell`中运行(比如避免`Shell`字符串篡改)，也可以使用`Exec`格式的`RUN`指令，`Exec`形式, 与`Shell`形式不同，`Exec`形式不会调用`/bin/sh -c`，这意味着正常的`Shell`处理不会发生。例如，`ENTRYPOINT [ "echo", "$HOME" ]`不会在`$HOME`上进行变量替换。如果想要进行`Shell`处理，可以使用`Shell`模式或直接执行命令，例如：`ENTRYPOINT echo "$HOME"`，而`ENTRYPOINT ["java", "-jar", "app.jar", "--spring.profiles.active=$RUN_ENV"]`不会进行`$RUN_ENV`的变量替换或者将其写在脚本里面，`ENTRYPOINT ["./entrypoint.sh"]`这样就可以进行变量替换了
