...
然而，OSI 模型过于庞大、复杂。参照此模型，技术人员开发了 TCP/IP 协议栈，简化 OSI 七层模型为 TCP/IP 四层模型。获得了更广泛的使用。
...
将 IP 地址和子网掩码转换成 2 进制的 32 位数字。例如 C 类 IP `192.160.4.1`。其子网掩码二进制为`11111111.11111111.11111111.00000000`，进行逻辑"与"(&)操作，得到网络 ID 为`192.160.4.0`。如果两个 IP 地址的网络 ID 相同，就说明他们在同一个子网中。
