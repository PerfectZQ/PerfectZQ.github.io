...
* **ElasticSearch使用手册：** [https://www.elastic.co/guide/en/elasticsearch/reference/current/index.html](https://www.elastic.co/guide/en/elasticsearch/reference/current/index.html)
...
linux 版本： kibana-6.0.0-linux-x86_64.tar.gz

　　解压之后，进入目录 `$KIBANA_HOME/config`，修改配置文件`kibana.yml`
```shell
...
## X-Pack 插件安装
* [X-Pack产品介绍](https://www.elastic.co/products/x-pack)
* [官方安装说明](https://www.elastic.co/guide/en/elasticsearch/reference/current/installing-xpack-es.html)

　　因为是在内网环境下搭建，无法通过外网直接安装x-pack，所以需要下载x-pack安装包。[https://artifacts.elastic.co/downloads/packs/x-pack/x-pack-6.0.0.zip](https://artifacts.elastic.co/downloads/packs/x-pack/x-pack-6.0.0.zip)。这个安装包包含了elasticsearch、logstash、kibana三个组件的x-pack安装包，安装时复用此文件即可。

　　首先在elasticsearch中安装X-Pack插件，在`$ES_HOME`目录下执行`bin/elasticsearch-plugin install file:///opt/elasticsearch/x-pack-6.0.0.zip`。x-pack需要一些权限，全部`y`就可以了。

　　然后是在kibana中安装X-Pack插件，在`$KIBANA_HOME`目录下执行`bin/kibana-plugin install file:///opt/elasticsearch/x-pack-6.0.0.zip`。
## ES Hadoop 
