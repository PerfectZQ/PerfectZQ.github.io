...
　　因为是在内网环境下搭建，无法通过外网直接安装X-Pack，所以需要下载[X-Pack](https://artifacts.elastic.co/downloads/packs/x-pack/x-pack-6.0.0.zip)安装包。这个安装包包含了elasticsearch、logstash、kibana三个组件的x-pack安装包，安装时复用此文件即可。

### 在elasticsearch中安装X-Pack插件
　　在`$ES_HOME`目录下执行`bin/elasticsearch-plugin install file:///opt/elasticsearch/x-pack-6.0.0.zip`。x-pack需要一些权限，全部`y`就可以了。或者在install的时候使用option`--batch`自动授权，`bin/elasticsearch-plugin install --batch file:///opt/elasticsearch/x-pack-6.0.0.zip`。

　　

### 是在kibana中安装X-Pack插件
　　在`$KIBANA_HOME`目录下执行`bin/kibana-plugin install file:///opt/elasticsearch/x-pack-6.0.0.zip`。优化和缓存浏览器的过程会稍微长一点，耐心等一下。
