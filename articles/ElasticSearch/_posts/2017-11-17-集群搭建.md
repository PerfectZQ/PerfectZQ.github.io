...
　　因为是在内网环境下搭建，无法通过外网直接安装x-pack，所以需要下载x-pack安装包。[https://artifacts.elastic.co/downloads/packs/x-pack/x-pack-6.0.0.zip](https://artifacts.elastic.co/downloads/packs/x-pack/x-pack-6.0.0.zip)。这个安装包包含了elasticsearch、logstash、kibana三个组件的x-pack安装包，安装时复用此文件即可。下载完成之后，在`$ES_HOME`目录下执行`bin/elasticsearch-plugin install file:///opt/elasticsearch/x-pack-6.0.0.zip`。x-pack需要一些权限，全部`y`就可以了。
## ES Hadoop 
