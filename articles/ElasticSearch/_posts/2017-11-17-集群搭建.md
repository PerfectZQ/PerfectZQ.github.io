...
### X-Pack 的功能的启用与禁止
　　X-Pack的功能默认是都启用的，可以在`$ES_HOME/config/elasticsearch.yml`禁用某些功能。
...
　　下面这些功能可以在`$ES_HOME/config/kibana.yml`中禁止，kibana页面中不会显示这些功能。
* **Reporting：** `xpack.reporting.enabled: false`。[详细配置项介绍](https://www.elastic.co/guide/en/kibana/6.0/reporting-settings-kb.html)
* **Security：** `xpack.security.enabled: false`。[详细配置项介绍](https://www.elastic.co/guide/en/kibana/6.0/security-settings-kb.html)
