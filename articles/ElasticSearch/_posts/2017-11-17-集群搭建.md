...
对于 7.0+ 的 Elasticsearch 自带X-pack(Note: elasticsearch-oss 版本是不带 x-pack 的)，所以直接配置即可
* [configuring-security](https://www.elastic.co/guide/en/elasticsearch/reference/7.7/configuring-security.html)
```shell
# rpm 默认安装目录
...
# 颁发证书，生成文件 elastic-certificates.p12
