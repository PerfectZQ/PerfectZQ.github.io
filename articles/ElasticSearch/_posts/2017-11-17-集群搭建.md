...
　　然后是在kibana中安装X-Pack插件，在`$KIBANA_HOME`目录下执行`bin/kibana-plugin install file:///opt/elasticsearch/x-pack-6.0.0.zip`。优化和缓存浏览器的过程会稍微长一点，耐心等一下。

　　X-Pack的功能默认是都启用的，可以在`$ES_HOME/config/elasticsearch.yml`禁止某些功能。
## ES Hadoop 
