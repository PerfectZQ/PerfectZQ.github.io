...
　　在`$ES_HOME`目录下执行`bin/elasticsearch-plugin install file:///opt/elasticsearch/x-pack-6.0.0.zip`。x-pack需要一些权限，全部`y`就可以了。或者使用`bin/elasticsearch-plugin install --batch file:///opt/elasticsearch/x-pack-6.0.0.zip`，使用option`--batch`自动授权。
