...
filter {
  if [type] == "logstash_person" {
    grok {
      match => { "message" => "%{WORD:name} %{DATE:birthday} %{IP:ip}" }
      remove_field => "message"
    }
  }
  else if [type] = "logstash_cat" {
    grok {
...
* auto_offset_reset: `earliest`: 从头开始消费，`latest`: 从最新的offset开始消费，该配置项没有默认值！
...
filter {
  grok {
      match => { "message" => "%{IP:ipaddress} %{WORD:username} %{NUMBER:age}" }
  }
}
