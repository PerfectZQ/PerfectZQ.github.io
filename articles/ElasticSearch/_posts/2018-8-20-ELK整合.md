...
* 如果在收割机关闭时移动或移除文件，则不会继续收集文件。

使用`close_ *`配置选项控制收割机何时关闭。
...
FileBeat 目前支持多种输入类型。每种输入类型都可以定义多次。日志输入检查每个文件以查看是否需要启动收集器，是否已经运行，或者是否可以忽略该文件（参考[ignore_older](https://www.elastic.co/guide/en/beats/FileBeat/current/FileBeat-input-log.html#FileBeat-input-log-ignore-older)）。如果自收割机关闭后文件的大小发生变化，则只会收集新行。
...
# FileBeat 会在注册表中存储每个文件收集的状态，想强制 FileBeat 从
# 日志的最开始重新读取，可以直接删除注册表文件
...
output {
    ...
    kafka {
        topic_id => "logstashtest"
        # 日志格式化
        codec => plain {
            format => "%{message}"
            charset => "UTF-8"
        }
        bootstrap_servers => "192.168.1.60:9092,192.168.1.61:9092"
    }
}
...
bin/logstash -f pipeline1.conf --config.reload.automatic
```