...
## Common
减少资源申请应该调 vcores, parallism / slots 向上取整 = taskmanager 数量，每个 taskmanager 一个 jvm，每个 slot 一个 subtask 对应一个处理线程, 默认一个 slot 申请一个yarn vcore，但是可以通过参数修改 TM 总的 vcores，而且 slots 数量是可以大于 vcores 数量的比如 slots = 8, vcores = 4，也就是JVM对应了总的 4个vcore 的cpu资源（简单理解为4核 8线程），vcore 都是yarn的概念，每个 nodemanager 上限都是配置死的，低优队列节点应该配的就是 40vcores 200g。flink_tm_vcores 实际上对应的应该就是 yarn.container.vcores