...
## kubernetes master
它提供一系列组件，例如：对外部 kubectl(user command) 提供 REST 接口，与内部集群进行交互的 Kubernetes API Server、用来创建和复制 Pod 的 Replication Controller、提供统一访问一组 Pods 的 Service...
...
* Kubelet：是主节点代理。
* Kube-proxy：Service 使用其将链接路由到Pod。
* Docker或Rocket：Kubernetes 用来创建容器的容器技术。
...
## Replication Controller
创建和复制 Pod，Replication Controller 确保任意时间都有指定数量的 Pod 副本在运行。如果为某个 Pod 创建了 Replication Controller 并且指定 3 个副本，它会创建3个Pod，并且持续监控它们。如果某个 Pod 不响应，那么 Replication Controller 会替换它，保持总数为 3 如下面的动画所示：
