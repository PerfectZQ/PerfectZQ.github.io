...
# 标准模式(standard): rebase 会自动将当前分支中的所有 commits 移动到
# passed branch(要合并到的分支)的分支头(Head)

# 交互模式(interactive): 可以单独更改流程中的各个 commits，而非移动所
# 有的 commits，使用 -i 进入交互模式

# 使用 rebase 之前保证所有处于 staging area 的文件都已 commit
$ git rebase
First, rewinding head to replay your work on top of it...
Applying: modify

# 在 rebase 期间，edit 或 e 命令将在该提交上暂停 rebase playback(回放)，Git 会打断回
