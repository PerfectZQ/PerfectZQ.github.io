...
# 从指定分支创建一个新的分支，并切换到新的分支，不指定分支，默认从当前的分支创建新分支
...
# 标准模式: rebase 会自动将当前分支中的所有 commits 添加到 <base> 的分支头(Head)，<base>默
# 认为当前分支的远程分支，如: refs/remotes/origin/master
# 换句话说，rebase 就是把我的变更 <current branch> 建立在所有其他人已做过的变更 <base> 之上
# 而 <base> 可以是任何类型的 commit reference(e.g.: commit id、branch name、a tag or 
