...
**A `Monoid` is any associative binary operation with a unit**. what's `associative`? involving the condition that a group of quantities connected by operators gives the same result whatever their grouping, as long as their order remains the same, e.g., ( a × b ) × c = a × ( b × c ).
