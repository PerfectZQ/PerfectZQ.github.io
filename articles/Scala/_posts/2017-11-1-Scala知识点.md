...
　　如果声明成员变量是用`private`关键词修饰时，生成的setter和getter方法也是`private`的。
```scala
class Person {
...
class Person {
    ...
    // setter方法
    private void name_(String name) {
        this.name = name;
    }
}
```
...
person = new Person
// 显式调用setter
person.name_("zhangsan")

// getter
val name = person.name

// 直接修改成员变量，实际上后台还是调用的 name_(String name) 方法。
// 别忘了生成的字节码文件中的name可是被关键字`private`修饰的。
person.name = "lisi"
```
...
### 多态
　　**多态(polymorphic)**又称**动态绑定(dynamic binding)**或者**延迟绑定(late binding)**。在代码执行期间而非编译期间确定所引用对象的实际类型(确切的子类型)，根据其实际的类型调用相应的方法。实际上就是将子类的引用赋值给父类的变量，在程序运行的时候去确定具体的子类，然后调用相应的方法。

　　多态遵守一个准则，即不管变量声明的类型是什么，调用方法时以引用对象的类型为准。什么意思呢？看下面的例子
```scala
class Person(var name:String, var age:Int){
  def squat() : Unit = {
    println("人蹲")
...
// 虽然声明变量p是Person类型，但是实际对象引用是Zombie类型的
...
我是秦始皇
```
　　虽然声明变量p是Person类型，但是实际对象引用是Zombie类型的，所以调用的方法也是Zombie类的方法。
