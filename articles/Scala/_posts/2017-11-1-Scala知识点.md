...
```
　　编译后发现`TraitTestAll.scala`生成了两个`.class`文件`TraitTestAll$class.class`和`TraitTestAll.class`，下面分别看看反编译后的内容，本例使用jd-gui(一款绿色的反编译工具)反编译：

　　首先看下`TraitTestAll$class.class`
