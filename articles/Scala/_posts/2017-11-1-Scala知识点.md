...
```
　　从上面可以看出`TraitTestAll`生成了一个`interface`和一个`abstract class`，在`interface`中将`具体成员变量`和`抽象成员变量`的`setter`和`getter`声明为`abstract`方法，但`具体成员变量`的`setter`用注解`@TraitSetter`标注，而且不论是`具体成员方法`还是`抽象成员方法`都声明为`abstract`类。然后在抽象类`TraitTestAll$class`中添加了2个静态方法去实现`具体成员变量`和`具体成员方法`。
　　
