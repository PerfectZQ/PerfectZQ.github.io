...
* **反向代理(Reverse Proxy)**：是指以代理服务器来接受客户端的连接请求，然后将请求转发给内部网络上的服务器，并将从服务器上得到的结果返回给 客户端，此时代理服务器对外就表现为一个反向代理服务器。**反向代理**代理的是服务器端，为服务器收发请求，使真实处理请求的服务器对客户端不可见、无感知。例如：一个稍微复杂写的系统其实是有很多服务构成的，客户端其实根本不需要知道这些，他只要把请求统一发到反向代理服务器的地址，代理就会根据配置自动找到能处理该请求的服务地址，并把处理结果返回给客户端。
