...
## Spark 日志存储
　　Spark 默认的日志输出文件存储在各个节点的`$SPARK_HOME/work`目录下。在`$SPARK_HOME/conf/log4j.property.template`中，有基本的控制台输出配置。`cp log4j.property.template log4j.property` 在`log4j.property`中可以添加自己的`Appender`增加其他的日志输出目标。
