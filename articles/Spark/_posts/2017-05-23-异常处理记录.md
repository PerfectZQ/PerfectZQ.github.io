...
* 如果必须引用`map`算子外部的对象，那么就让这个对象在 executor 上初始化，即在local execution context中创建对象。例如创建一个 redis 的 client，该对象就没有实现`Serializable`接口：
...
```
* 所以如果是引用了 class 的成员函数导致整个 class 需要支持序列化，可以用`@transient`声明该成员不需要被序列化。
* 通过将此成员函数放在 Scala 的 object 中，类似与 Java 的 static，不要在类加载的时候就初始化对象，而是在 executor 真正去执行的时候去初始化对象。
