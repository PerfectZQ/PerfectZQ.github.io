...
/**
  ...
  * JVM 会在我们尝试实例化(new、clone、deserialize、reflect...)一个对象的时候，先检测相关类是否已经加载并
  * 初始化，如果没有加载，则通过 ClassLoader.loadClass() 将相关类(.class 文件)依次加载到内存。
  * 
  * 注: 使用这种方式需要确保 RedisUtil 在 Executor 上执行之前没有被 Driver JVM 加载
  * 过，即 Driver 端没有代码调用过 RedisUtil 类
  */
object RedisUtil {
  // Scala 没有 static 关键字，object 中的成员变量实际上都是 static 的，而类中的 static 成员(变量和方法)在
  // 类加载进内存的时候按顺序从上往下执行。这也是为什么 static 成员不能访问非 static 成员，反过来却可以的原因，
  // 因为 static 成员初始化的早，非 static 成员是在类加载完成后才开始初始化的
  private val redis: Jedis = new Jedis("10.4.121.202", 6379)
}
...
object Test {
  def main(args: Array[String]): Unit = {
    ...
    resultRDD.map {
        case (key: String, nameAndIP: String) =>
          // Executor JVM 在使用 RedisUtil 的时候会进行加载，且 JVM 内部的机制能够保证类的加载过程是线
          // 程互斥的
