...
　　但是在算子的计算过程中难免需要使用外部变量，要使用这些外部变量，就需要对这些外部变量做好序列化工作。最普遍的情形是：当引用了某个类（绝大多数是当前class或者object）的某个成员变量或者成员函数的时候，将导致这个类的所有的成员都需要支持序列化。即便这个类extends了Serializable，声明支持序列化，如果某些字段(成员对象，Object)不支持序列化，仍然会导致这个类在序列化的时候出现问题。可以将类中不能被序列化的成员变量标注为`@transient`，表示不对其进行序列化。
...
```scala
object RedisUtil {
  // 当clients被调用的时候初始化，调用init()方法
  lazy val clients: JedisCluster = init()
...
object LogAnalyseAndStatistics {
  def main(args: Array[String]): Unit = {
    // 当任务分配到各个executor上去执行的时候才会初始化，这样就不需要反序列化了。
    lazy val clients = RedisUtil.clients
    resultRDD = ...
    resultRDD.foreach {
