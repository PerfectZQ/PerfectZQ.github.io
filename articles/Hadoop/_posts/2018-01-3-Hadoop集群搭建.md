...
><font style="color: indianred;">WARNING: hdfs namenode -format 会重置 namenode 保存的所有元数据，这意味着，即便你的数据存储在 datanode 中，如果没有 namenode 中的元数据信息，hdfs 就无法找到真实数据的存放地址，这和删掉所有数据没什么区别。如果你处于生产环境，千万不要使用该命令！</font>
